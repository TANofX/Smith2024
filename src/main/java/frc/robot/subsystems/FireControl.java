...
public class FireControl extends SubsystemBase {
  ...
  public void periodic() {
     ...
     if(DriverStation.Alliance.Blue == DriverStation.getAlliance().get()) {
         distanceFromSpeaker = shooterPose.getTranslation().getDistance(Constants.FireControl.BLUE_SPEAKER_POSITION.getTranslation()); //Finding the shooter's distance from the robot when on blue alliance
     }
    else {
         distanceFromSpeaker = shooterPose.getTranslation().getDistance(Constants.FireControl.RED_SPEAKER_POSITION.getTranslation());//Finding the shooter's distance from the robot when on red alliance
     }
     ...
     double shooterVelocity = Math.sqrt(2*Constants.FireControl.ACCELERATION*Constants.FireControl.HEIGHT + Math.pow(xShootervelocity, 2)); //vector sum to find overall shooter velocity
     double shooterAngle = Math.atan(Constants.FireControl.ACCELERATION*distanceFromSpeaker); //Finding the angle the shooter needs to shoot into the speaker from its current position

     //Below are the testing code to verify projectile motion equations
